...
class Products
{
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/pro/product_images/".$uniqueImageName)) {
					...
					if ($q) {
						...
						return ['status'=> 303, 'message'=> 'Produto modificado com sucesso!'];
					}
	...
	public function editProductWithoutImage($pid,
		...
		if ($pid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Produto actualizado com sucesso'];
			}else{
				return ['status'=> 303, 'message'=> 'Falha ao executar a consulta'];
			}
	...
	public function addCategory($name){
		...
		if ($q->num_rows > 0) {
			return ['status'=> 303, 'message'=> 'Categoria já existe'];
		}else{
	...
	public function deleteProduct($pid = null){
		if ($pid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Produto retirado dos stocks'];
			}else{
	...
	public function deleteCategory($cid = null){
		if ($cid != null) {
			...
			return ['status'=> 303, 'message'=>'ID de categoria inválida'];
		}
	...
	public function addBrand($name){
		...
		if ($q->num_rows > 0) {
			return ['status'=> 303, 'message'=> 'Marca já existe'];
		}else{
